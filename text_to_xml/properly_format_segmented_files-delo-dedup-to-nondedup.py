...
def change_header_part(tree, original_filename):
    ...
    base_out_filename = "./ver3_delo_dedup_nondedup/"+original_filename
...
def print_missing_gigafida_paragraphs(things_in_curr_paragraph, part, outf):
    first_paragraph = True
    for e in things_in_curr_paragraph:
        if is_next_part_a_sent(e):
            print_sent_xml(e, outf, part)
...
def find_missing_gigafida_paragraphs(lines, parsed_gigafida_original, part, outf):
    parsed_gigafida = copy.copy(parsed_gigafida_original)
    first_paragraph = True
    segments_to_insert = []
    last_popped_paragraph = None
    total_prints = 0
    og_part = part
    things_in_curr_paragraph = []
    for i, line in enumerate(lines):
        ...
        while True:
            if len(parsed_gigafida) == 0:
                return parsed_gigafida
            e = parsed_gigafida[0]
            things_in_curr_paragraph.append(e)
            if is_next_part_a_sent(e):
                
                merged_sents_from_xml.append("".join(get_sent_text(e)))
                merged_sent_len += 1
                parsed_gigafida.pop(0)
            elif not is_next_part_a_paragraph(e):
                parsed_gigafida.pop(0)
                ...
                if first_paragraph:
                    ...
                    if "".join(merged_sents_from_xml) != line:
                        segments_to_insert.append(merged_sents_from_xml)
                        if merged_sent_len > 1:
                            #print(part, merged_sent_len, "missing", merged_sents_from_xml)
                            #for t in things_in_curr_paragraph:
                            #    print(t)
                            
                            current_outf_filename = "/".join(outf.name.split('/')[:-1])+'/'+outf.name.split('/')[-1].split('-')[0]+'-'+str(part)+'-missing-'+str(part-og_part+1)+'.xml'
                            part += 1
                            #print(current_outf_filename)
                            with open(current_outf_filename, 'w', encoding='utf-8') as missing_outf:
                                print_missing_gigafida_paragraphs(things_in_curr_paragraph[:-1], part, missing_outf) 
                                for t in things_in_curr_paragraph[:-1]:
                                    parsed_gigafida_original.remove(t)
                            things_in_curr_paragraph = [e]
                        #else:
                        #    print(part, merged_sent_len, "missing", merged_sents_from_xml)
                        merged_sents_from_xml = []
                        merged_sent_len = 0
    ...
    return parsed_gigafida_original
...
def merge_lines_with_gigafida_paragraphs(lines, parsed_gigafida, outf, part):
    ...
    for i, line in enumerate(lines):
        ...
        while True:
            ...
            """
            if len(parsed_gigafida) == 0:
                if len(sents_to_print_new) == 0:
                    print("Empty file", outf.name)
                    outf.close()
                    os.remove(outf.name)
                    return parsed_gigafida
                    #exit()
            ...
            if is_next_part_a_sent(e):
                ...
                merged_sents_from_xml += "".join(get_sent_text(e))
                merged_sent_len += 1
                #print('wrote', "".join(get_sent_text(e)))
                parsed_gigafida.pop(0)
            elif not is_next_part_a_paragraph(e):
                #should be fs
                #print(e, file=outf)           
...
#segmented_folder = './Segmentacija_dnevnik_vse/'
...
for filename in files_in_segmented_folder:
    ...
    #if filename != '98_GF2848187.xml' and not already_skipped:
    #    continue
    ...
    if '-' in filename:
        base_filename = filename.split("_")[1].split("-")[0]+".xml"
    else:
        base_filename = filename.split("_")[1]
        ...
        for i, line in enumerate(segmented_file):
            ...
            if line[:5] == "-----":
                ...
                with open(curr_filename, "w", encoding="utf-8") as outf:
                    ...
                    find_missing_gigafida_paragraphs(lines_in_curr_segment,  parsed_gigafida, current_segment, outf)
                    #print(len(parsed_gigafida))
